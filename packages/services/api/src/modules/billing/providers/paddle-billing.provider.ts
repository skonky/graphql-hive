...
export class PaddleBillingProvider implements BillingDataProvider {
  ...
  async generatePaymentMethodUpdateToken(customerId: string): Promise<string> {
  ...
  async updateBillingDetails(customerId: string, details: BillingInfoUpdateInput) {
  ...
  private async handleSubscriptionCreatedEvent(event: SubscriptionCreatedEvent) {
    ...
    if (customData && customData.hiveSubscription && customData.organizationId) {
      ...
      // Step 6: Update the customer name in Paddle because it's empty by default
      await this.service.updateCustomerName.mutate({
        customerId: event.data.customerId,
  ...
  async syncOperationsLimit(customerId: string, operationsInMillions: number): Promise<void> {
  ...
  async subscriptionManagementUrl(customerId: string): Promise<string | null> {
    const activeSubscription = await this.service.activeSubscription.query({
      customerId,
    });
  ...
  async hasPaymentIssues(customerId: string): Promise<boolean> {
    const activeSubscription = await this.service.activeSubscription.query({ customerId });
  ...
  async invoices(customerId: string): Promise<BillingInvoice[]> {
    const invoices = await this.service.invoices.query({ customerId });
  ...
  async upcomingInvoice(customerId: string): Promise<BillingInvoice | null> {
    const nextTransaction = await this.service.upcomingInvoice.query({
      customerId,
    });
    ...
    return {
      ...
      amount: nextTransaction?.details.totals.total
        ? parseFloat(nextTransaction.details.totals.total)
        : 0,
      date: new Date(nextTransaction?.billingPeriod.startsAt ?? Date.now()),
      periodStart: new Date(nextTransaction?.billingPeriod.startsAt ?? Date.now()),
      periodEnd: new Date(nextTransaction?.billingPeriod.endsAt ?? Date.now()),
      status: 'OPEN',
      pdfUrl: null,
    };
  }

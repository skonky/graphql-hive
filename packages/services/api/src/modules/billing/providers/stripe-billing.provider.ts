...
export class StripeBillingProvider implements BillingDataProvider {
  ...
  async upcomingInvoice(customerId: string): Promise<BillingInvoice | null> {
    ...
    return {
      id: String(upcomingInvoice.number!),
      date: new Date(upcomingInvoice.created),
      amount: upcomingInvoice.total!,
      periodStart: new Date(upcomingInvoice.period_start),
      periodEnd: new Date(upcomingInvoice.period_end),
      status: upcomingInvoice?.status
        ? (upcomingInvoice.status.toUpperCase() as BillingInvoiceStatus)
        : 'DRAFT',
      pdfUrl: upcomingInvoice.invoice_pdf!,
    };
  }

...
export class BillingProvider {
  ...
  async downgradeToHobby(billingRecord: OrganizationBilling) {
    ...
    if (billingRecord.provider === 'PADDLE') {
      return await this.paddle.cancelActiveSubscription(billingRecord.externalBillingReference);
    }
  ...
  async getActiveSubscription(billingRecord: OrganizationBilling): Promise<Subscription | null> {
    if (billingRecord.provider === 'PADDLE') {
      return this.paddle.getActiveSubscription(billingRecord.externalBillingReference);
    }
  ...
  async updateBillingDetails(billingRecord: OrganizationBilling, input: BillingInfoUpdateInput) {
    if (billingRecord.provider === 'PADDLE') {
      return await this.paddle.updateBillingDetails(billingRecord.externalBillingReference, input);
    }
  ...
  > {
    if (billingRecord.provider === 'PADDLE') {
      return {
        ...
        ...(await this.paddle.billingInfo(billingRecord.externalBillingReference)),
      };
    }
  ...
  async hasPaymentIssues(billingRecord: OrganizationBilling): Promise<boolean> {
    if (billingRecord.provider === 'PADDLE') {
      return this.paddle.hasPaymentIssues(billingRecord.externalBillingReference);
    }
  ...
  async invoices(billingRecord: OrganizationBilling): Promise<BillingInvoice[]> {
    if (billingRecord.provider === 'PADDLE') {
      return this.paddle.invoices(billingRecord.externalBillingReference);
    }
  ...
  async upcomingInvoice(billingRecord: OrganizationBilling): Promise<BillingInvoice | null> {
    if (billingRecord.provider === 'PADDLE') {
      return this.paddle.upcomingInvoice(billingRecord.externalBillingReference);
    }

    if (billingRecord.provider === 'STRIPE') {
      return this.stripe.upcomingInvoice(billingRecord.externalBillingReference);
    }
  ...
  async subscriptionManagementUrl(billingRecord: OrganizationBilling): Promise<string | null> {
    if (billingRecord.provider === 'PADDLE') {
      return this.paddle.subscriptionManagementUrl(billingRecord.externalBillingReference);
    }

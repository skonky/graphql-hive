...
async function getActiveSubscription(
  ...
  const subscriptions = await paddle.subscriptions
    ...
    .next()
    .then(v => v.filter(r => r.customData && 'hiveSubscription' in r.customData));
...
export const paddleBillingApiRouter = t.router({
  ...
  upcomingInvoice: procedure
    ...
    .query(async ({ ctx, input }) => {
      const activeSubscription = await getActiveSubscription(ctx.paddle, input.customerId);

      if (!activeSubscription) {
        return null;
      }

      return activeSubscription.nextTransaction ?? null;
    }),
  ...
  activeSubscription: procedure
    ...
    .query(async ({ ctx, input }) => {
      return await getActiveSubscription(ctx.paddle, input.customerId);
    }),
  updateBillingContact: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      const actualSubscription = await getActiveSubscription(ctx.paddle, input.customerId);
  ...
  updateCustomerName: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.paddle.customers.update(input.customerId, {
        name: input.name,
      });
    }),

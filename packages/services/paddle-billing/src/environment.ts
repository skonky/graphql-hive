...
const PaddleModel = zod.object({
  PADDLE_API_KEY: zod.string(),
  PADDLE_ENVIRONMENT: zod.union([zod.literal('sandbox'), zod.literal('production')]),
  PADDLE_WEBHOOK_SECRET: zod.string().min(1),
});
...
const configs = {
  ...
  // eslint-disable-next-line no-process-env
  paddle: PaddleModel.safeParse(process.env),
  // eslint-disable-next-line no-process-env
...
const paddle = extractConfig(configs.paddle);
...
export const env = {
  ...
  paddle: {
    apiKey: paddle.PADDLE_API_KEY,
    environment: paddle.PADDLE_ENVIRONMENT,
    webhookSecret: paddle.PADDLE_WEBHOOK_SECRET,
  },
  ...
  prometheus:
    prometheus.PROMETHEUS_METRICS === '1'
      ? {
          labels: {
            instance: prometheus.PROMETHEUS_METRICS_LABEL_INSTANCE ?? 'paddle-billing',
          },
        }
      : null,
} as const;
